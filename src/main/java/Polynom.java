...
public class Polynom {
    ...
    public List<Integer> getIndexOfOne(){
        List<Integer> res = new ArrayList<>();
        for(int i = 0 ; i < this.getPolymomCoefficient().length ; i ++){
            if(this.getPolymomCoefficient()[i]==1) res.add(this.getPolymomCoefficient().length-i-1);
        }
        res.remove(0);
        return res;
    }

    public static List<Integer> getIndexOfOne(Polynom p){
        List<Integer> res = new ArrayList<>();
        for(int i = 0 ; i < p.getPolymomCoefficient().length ; i ++){
            if(p.getPolymomCoefficient()[i]==1) res.add(p.getPolymomCoefficient().length-i-1);
        }
        res.remove(0);
        return res;
    }
